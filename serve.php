...
$observer->setFetchContactObserver(function(array $contacts){
    ...
    foreach($contacts as $k => $v) {
        if($k == 'friends'){
            foreach($v as $vv){
                $pdo->exec("insert into friends(UserName,NickName,RemarkName,HeadImgUrl,CreateTime,UpdateTime) values('".$vv["UserName"]."','".$vv['UserName']."','".$vv['UserName']."','".$vv['UserName']."','".date("Y-m-d H:i:s",time())."','".date("Y-m-d H:i:s",time())."')");
            }
        };
    }
